...
const ADDONS = [
  ...
  { id: "salada-fresca", title: "Salada Fresca", price: 7.9, description: "Mix de folhas verdes com legumes.", category: "acompanhamentos" },
...
function getLocalSiteData() {
  ...
  return { ...DEFAULT_SITE_DATA, ...saved };
}
...
function buildPrintPayload(customer, selectedItemIds, addonIds, totals) {
  ...
  const modifiers = [...selectedAddons.map((addon) => addon.title), customer.note].filter(Boolean);
